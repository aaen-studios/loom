...
export function LiquidSurface({
  ...
  */
  ...
  /*
    ...
    The table is the default and the only caller that overrides it is the preview
    in Settings, which *has* to: a preview exists to demonstrate the effect, and
    at the opacity the real surfaces need for legibility there is nothing visible
    through them to bend. Every other call site takes its group's number, so no
    surface has to rediscover that a popover must stay readable.

    A group at `100` means "the token exactly as it was", which is what keeps a
    converted surface indistinguishable from its unconverted self unless you look
    at the rim.
  */
