...
| Layout | Full-bleed background, centered full-width canvas. **No permanent sidebar** — the chats list is a dock panel (see Dock below), **closed on launch like every other panel**, and one click or one `Ctrl+`` away. Zones overlay the transcript rather than taking width from it, so an open panel never moves the chat. No big blurred panel around the workspace. |
...
| Git | A panel: commit box with an **AI message button** at the top, then the branch and remote row, then staged and unstaged file lists, then history. `crates/loom-core/src/git.rs` drives the **system `git` CLI** through the same `process::hidden_std` path the agent's `git_*` tools use, so there is one implementation rather than two — and Git Credential Manager, the user's config, hooks, aliases, SSH keys, signing and LFS all simply work, because it *is* their git. Status is parsed from `--porcelain=v2 -z -b`, the only form that gives honest unmerged codes, machine-readable rename pairs and paths that survive a newline. Every write calls the backend and **adopts the status that comes back** rather than modelling what staging does to a two-sided file: a speculative version would be wrong in the dangerous direction, showing a file as staged when it is not. Refreshes on a workdir change, window focus, `loom://fs`, and a finished operation — **never on a timer**, because a poll would spawn a process per few seconds for a panel that is usually not visible. `fetch`/`pull`/`push` get a 90s deadline because with stdin closed a credential prompt cannot be answered, and a timeout with a sentence beats a hang; `pull` passes `--rebase` so a local commit followed by a pull does not produce a merge commit. Committing with nothing staged, over a conflict, or with an empty message is refused in Rust, and `discard` is the one action that always cards — uncommitted bytes are the one thing nothing can restore. |
| Editor | **Monaco**, in a dock panel and in IDE mode, with `FileTree` as the files pane. The panel is realcode: find/replace, folding, multi-cursor, bracket guides, and a command palette, in a theme derived from Loom's own CSS variables — resolved through a hidden element's computed `color`, because Monaco's theme parser does not accept `color-mix()`. **Diffs are editor tabs, not a patch renderer**: clicking a changed file opens `diff:` and two file versions go to `Monaco.DiffEditor`, which is exact where a patch is a description. **Buffers live outside React** in `lib/editors.ts`, keyed by path, because `PanelBody` mounts only the active tab — a model in component state would be destroyed on every panel switch, with unsaved edits in it. The same relationship `lib/terminals.ts` has with xterm, for the same reason. **Autosave is on by default and is only safe because of the hash guard**: a write carries the SHA-256 of what was loaded and `edit::write_text` refuses if the file moved on, returning a conflict instead of writing. A dirty buffer that changed underneath raises the reload/keep banner; a clean one reloads silently. CRLF, BOM and lossy-UTF-8 are detected on read and restored on write, because an editor that silently rewrites line endings has corrupted the file in a way a diff shows as every line changed. Deadlock-free by construction: `git` is spawned with its pipes drained on their own threads, since reading them only after the child exits is the classic way to hang a pipe that fills. |
| IDE mode | A title-bar button that swaps the dock's **centre child**: `<DockHost>{ideMode ? <IdeShell/> : <ChatCanvas/>}</DockHost>`. That one line is why the terminal still docks and still tears off, `Ctrl+`` still toggles, and a bottom Runs panel behaves identically in both modes — a second application shell would have reimplemented all of it and then drifted. `IdeShell` is a file tree and git panel on the left, the editor in the middle, and the chat as a **slim right-hand column you can widen or hide** — never removed, because the model is the reason the app exists and an editor that hides the composer makes you leave it to ask a question. The sidebar renders the *same* `GitPanel` and `FileTree` the dock does, so there is one of each. Persisted in `interface.ideMode`, a deliberate departure from the dock's "closed every launch" rule: a panel is something you *ask* to see, whereas a way of working is something you have said. **No keyboard shortcut**, and Monaco is the reason — it claims most of `Ctrl+<letter>`, and a global binding that silently loses to an editor-local one works everywhere except where it was added for. |
| Files changed | `EngineEvent::FilesChanged` fires after **every** tool call, and `loom://fs` after every IDE command that can write. Two channels because they answer who did it: the first is a tool, the second is the user. Both are over-approximations, deliberately — deciding whether a given call actually changed anything cannot cover an MCP server's tools, and what the event really asks ("restat what you have open") is cheap and always correct. The editor re-stats its buffers behind a size-and-mtime **hint**, so the common case is one string compare and only a file that looks different pays for the hash. `git` is never consulted by the file tree; the letters `M`/`A`/`D`/`U`/`C` come from the git store, so the tree cannot disagree with the panel beside it. |
...
- ~~`Files` shows the workspace file list, not a change set. There is no git diff
  command, and deriving one from a list of paths would be inventing it.~~ **Done.**
  `Files` is now `FileTree` — an expandable tree that opens files in the editor —
  and the change set lives in the Git panel, which needs no diff command of its
  own because `git_file_at` hands the editor two file versions and Monaco's
  `DiffEditor` does the rest. The old limitation was true of a *list*; it stopped
  being the whole story when the backend grew `dir_list`, `file_read` and
  `git_file_at`.
- The editor has now been **driven by hand in a running window**, which settles
  the question that mattered most: `scripts/probe-editor.mjs` reports
  `typescript` and `editorWorkerService` workers constructed with no errors, so
  Monaco is not silently falling back to the main thread under the CSP. That
  probe needed two fixes of its own to be worth believing — it opens a JS/TS
  file rather than `package.json`, because JSON's language service never asks
  for a worker, and it waits for the dock to move to the chosen session's folder
  before opening the panel, because the folder's stored layout lands *after*
  `openSession` resolves and would otherwise remove it. What is still only
  eyeballed: that the diff view reads well at a 460px docked width, that
  autosave does not fight fast typing, and that the splitter drags feel right.
  `git.rs` and `edit.rs` tests, `fileTree.test.ts`, and `cargo check`/`tsc` are
  green.
- The Git panel has **no diff view of its own and should not grow one**. Clicking
  a changed file opens a `diff:` tab in the editor. If a future change adds a patch
  renderer to the panel, there will be two implementations of the same idea and
  they will disagree about a file with no trailing newline.
...
- The canvas is full-width and centered; the chats list is a dock panel, closed by default.
